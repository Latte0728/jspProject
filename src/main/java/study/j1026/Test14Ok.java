...
public class Test14Ok extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Front에서 넘어온 값을 변수에 담아준다. (WAS에서 Web Server에게 요청)
		String name = request.getParameter("name")==null ? "" : request.getParameter("name"); // header에 들어 있음
		int age = (request.getParameter("age")==null || request.getParameter("age").equals("")) ? 0 : Integer.parseInt(request.getParameter("age")); // header에 들어 있음
		String gender = request.getParameter("gender")==null ? "남자" : request.getParameter("gender"); // header에 들어 있음
		String[] hobbys = request.getParameterValues("hobby"); // header에 들어 있음
		String job = request.getParameter("job"); // header에 들어 있음
