...
<div class="container">
	...
	<pre>
		사용법1 : < c : forEach var="변수" begin="초기값" end="최종값" step="증감값" varStatus="매개변수" >
						< /c : forEach >
		사용법2 : < c : forEach var="변수" items=" $ {객체명/배열}" varStatus="매개 변수" > <!-- items="{객체명/배열}" : 자바의 향상된 for문과 같다. -->
						< /c : forEach >
		사용법3 : < c : forEach var="변수" items=" $ {객체명/배열}" delims="구분 기호"> <!-- delims : 구분자. -->
						< /c : forEach >								
	</pre>
	<p>사용법1 :<br/>
		<%-- <c:forEach var="i" begin="1" end="10" step="1"> step 1일 경우는 생략 가능 --%>
...
%>		
	...
	<p>각종 사용 예제<br/>
		...
		<br/><br/>
		-문제1: 모든카드를 출력하되, 국민카드는 파란색, 삼성카드는 빨간색으로 출력하시오.
		<%-- <c:forEach var="card" items="${cards}">
		${card} / 
	</c:forEach> 모든 카드 출력 코드--%>
	<c:forEach var="card" items="${cards}">
	...
	<c:forEach var="card" items="${cards}" varStatus="st">
		...
		<c:forEach var="card" items="${cards}" varStatus="st">
		<c:choose>
			<c:when test="${st.first}"><font color='red'></c:when>
			<c:when test="${st.last}"><font color='blue'></c:when>
	...
	<c:forEach var="i" begin="2" end="8" varStatus="st">
